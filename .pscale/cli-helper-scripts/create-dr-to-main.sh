...
wait-for-deploy-request-merged.sh 9 "$DB_NAME" "$DEPLOY_REQUEST_NUMBER" "$ORG_NAME"

. .pscale/cli-helper-scripts/wait-for-deploy-request-merged.sh 9 "$DB_NAME" "$DEPLOY_REQUEST_NUMBER" "$ORG_NAME"
